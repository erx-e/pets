...
@Component({
  ...
  templateUrl: "./encuentra.component.html",
  styleUrls: ["./encuentra.component.scss"],
})
export class EncuentraComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.postpetService.getByState("E", 10, 0).subscribe((data) => {
  ...
  applyFilter(event: SubmitEvent) {
    if (this.form.valid) {
      if (event.submitter.classList.contains("btn-apply")) {
        ...
        this.postpetService
          .getByState("E", this.limit, this.offset)
          .subscribe((data) => {
            this.postspet = this.postspet.concat(data);
            this.notMorePostpet = false;
            this.filter = false;
